namespace PCHCB.Data.Configurations
{
    ...
    public class RamEntityConfiguration : IEntityTypeConfiguration<Ram>
    {
        public void Configure(EntityTypeBuilder<Ram> builder)
        {
            builder
                .HasOne(p => p.Provider)
                .WithMany(r => r.ProviderRams)
                .HasForeignKey(p => p.ProviderId)
                .OnDelete(DeleteBehavior.Restrict);

            builder.HasData(GenerateRams());
        }

        private Ram[] GenerateRams()
        {
            ICollection<Ram> rams = new HashSet<Ram>();
